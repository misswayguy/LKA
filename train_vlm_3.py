...
def main(args):
    ...
    # if os.path.exists("fiftyone_five_weights_sw_tiny_covid_full") is False:
    #     os.makedirs("fiftyone_five_weights_sw_tiny_covid_full")
    ...
    #     for class_name in os.listdir(root_dir):
    #         class_dir = os.path.join(root_dir, class_name)
    #         images_subdir = os.path.join(class_dir, "images")  # 👈 多加一层
    ...
    #     labels = []
    #     class_names = sorted(os.listdir(root_dir))  # 保证类名和下标一一对应
    #     class_to_idx = {class_name: idx for idx, class_name in enumerate(class_names)}
    #     for img_path in image_paths:
    #     # 从路径中提取类别名（即两级目录上）
    ...
    for epoch in range(args.epochs):
        ...
        # if val_acc > acc_best:
        #     torch.save(model.state_dict(), "/mnt/data/lsy/ZZQ/fiftyone_five_weights_sw_tiny_covid_full")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-path', type=str,
                      default="/mnt/data/lsy/ZZQ/cifar-100_cross_validation/train_10")
    ...
    parser.add_argument('--device', default='cuda:2', help='device id (i.e. 0 or 0,1,2,3,4,5 or cpu)')
