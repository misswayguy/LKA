...
from lora_layers import LoRA_Adapter
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.):
        ...
        # LoRA
        self.lora_q = LoRA_Adapter(in_dim=dim, bit=32)
        self.lora_v = LoRA_Adapter(in_dim=dim, bit=32)
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        # LoRA shortcut
        delta_q = self.lora_q(x).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        delta_v = self.lora_v(x).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
