...
class Seven(nn.Module):
    def __init__(self, dim=8, xavier_init=False):
        ...
        #self.adapter_conv = nn.Conv2d(dim, dim, 5, 1, 2)
...
class Block(nn.Module):
    def __init__(self,
        ...
        self.seven = Seven(dim)

    def forward(self, x):
        shortcut = x
        x = self.norm1(x)
        shortcut_2 = self.drop_path(self.seven(x))
        x = shortcut + self.drop_path(self.attn(x)) + shortcut_2

        identity = x
        x = self.norm2(x)
        identity_2 = self.drop_path(self.seven(x))
        x = self.mlp(x)
