...
class Mona(nn.Module):
    def __init__(self,
        ...
        self.project1 = nn.Linear(in_dim, 10)
        self.nonlinear = F.gelu
        self.project2 = nn.Linear(10, in_dim)

        self.dropout = nn.Dropout(p=0.1)

        self.conv1 = nn.Conv2d(in_channels=in_channels, out_channels=in_channels, kernel_size=31, stride=1, padding=15, groups=in_channels)
        self.conv2 = nn.Conv2d(in_channels=in_channels, out_channels=in_channels, kernel_size=5, stride=1, padding=2, groups=in_channels)
    ...
    def forward(self, x, hw_shapes=None):
        ...
        conv1_out = self.conv1(project1)
        conv2_out = self.conv2(project1)
        project1 = conv1_out + conv2_out
...
# class Mona(nn.Module):
#     def __init__(self, input_dim, middle_dim=10, reduction=0.25):
#         super(Mona, self).__init__()
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        self.my_module_1 = Mona(dim, 10)
        self.my_module_2 = Mona(dim, 10) # Adapter_FFN(dim, 8)
    ...
    def forward(self, x, attn_mask):
        ...
        x = self.norm1(x)
        shortcut_2 = self.drop_path(self.my_module_1(x,(H,W)))
        x = x.view(B, H, W, C)
        ...
        # FFN
        x = shortcut + self.drop_path(x) + shortcut_2
        identity = x
        x = self.norm2(x)

        identity_2 = self.drop_path(self.my_module_2(x,(H,W)))

        x = self.mlp(x)
        x = identity + self.drop_path(x) + identity_2
