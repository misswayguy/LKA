...
def create_train_set(root: str, train_ratio: float, fixed_test_path: str, fixed_count: int = None):
    """
    根据指定训练集比例或固定数量创建训练集。
    :param root: 数据集根目录。
    :param train_ratio: 训练集比例 (0.0-1.0)。
    :param fixed_test_path: 测试集文件路径。
    :param fixed_count: 固定训练集数量。如果指定，则覆盖比例。
    :return: 训练集图片路径和标签。
    """
    assert os.path.exists(root), f"Dataset root: {root} does not exist."
    ...
    for cla in classes:
        ...
        if fixed_count:  # 如果设置了固定数量，直接选取
            train_samples = train_candidates[:fixed_count]
        else:  # 按比例选择
        ...
        # 如果达到固定数量，直接停止
        if fixed_count and len(train_images) >= fixed_count:
            train_images = train_images[:fixed_count]
            train_labels = train_labels[:fixed_count]
            break
...
def main(args):
    ...
    if os.path.exists("/mnt/data/lsy/ZZQ/weights_base_covid_10_image") is False:
        os.makedirs("/mnt/data/lsy/ZZQ/weights_base_covid_10_image")
    ...
    # 创建固定数量的训练集
    # 创建训练集：固定为 10 张图片
    train_images_path, train_images_label = create_train_set(
        args.data_path, train_ratio=args.train_ratio, fixed_test_path=fixed_test_path, fixed_count=10
)
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > acc_best:
            torch.save(model.state_dict(), "/mnt/data/lsy/ZZQ/weights_base_covid_10_image_best.pth")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default='cuda:1', help='device id (i.e. 1 or 0,1,2,3,4,5 or cpu)')
    parser.add_argument('--train-ratio', type=float, default=0.0, help='training set ratio (e.g., 0.1 for 10%)')
    parser.add_argument('--fixed-count', type=int, default=10, help='number of training samples (overrides ratio)')
